...
class App:
    ...
    def callback(self, direction):
        ...
        if self._canclick:
            ...
            # Check if there is space to spawn a new Tile
            if not 0 in self.table:
                self._lose()
                return

            if backup != self.table:
                # Waits until the Tiles stop and spawns a new one
    ...
    def _griddraw(self):
        ...
        for n in range(1, 4):
            # Vertical lines
            self.main_canvas.create_line(n * Tile_width, 0, n * Tile_width, WIDTH, fill=line_color)
            # Horizontal lines
            self.main_canvas.create_line(0, n * Tile_width, WIDTH, n * Tile_width, fill=line_color)

    def _Tilesetup(self, nstart):
    ...
    def _spawnnew(self):
        """ Creates a new Tile and draws it in an empty space """
        ...
        while True:
            ...
            self.table[pos] = Tile.Cell(self.main_canvas, self.root, posconv, WIDTH / 4, n=2 ** ri(1, 3))
            break
    ...
    def _down(self):
        """ Moves all the Tiles to the bottom """
    ...
    def _up(self):
        """ Moves all the Tiles to the top """
